...
class _SchoolDetailScreenState extends State<SchoolDetailScreen> {
  ...
  Future<void> _fetchSchoolContributions() async {
    final response = await http.post(
      Uri.parse('http://116.124.191.174:15023/selected-school-contributions'),
  ...
  List<PieChartSectionData> _generateSchoolContributionChart() {
    if (widget.totalTime == 0) return [];

    return userContributions.map((user) {
      final userTime = user['total_time'] ?? 0;
      final contributionPercent = (userTime / widget.totalTime) * 100;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: true,
        title: Text('${widget.schoolName} 정보'),
      ),
      body: Container(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 10),
              Text('총 순위: ${widget.totalRanking}위'),
              SizedBox(height: 10),
              Text('총 누적 공부 시간: ${_formatTime(widget.totalTime)}'),
              SizedBox(height: 20),
              Divider(),
              Text(
                '학교 기여도 분포',
              ...
              Flexible(
                child: userContributions.isEmpty
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final contributionPercent = (user['total_time'] / widget.totalTime) * 100;
                    final color = Colors.primaries[userContributions.indexOf(user) % Colors.primaries.length];
